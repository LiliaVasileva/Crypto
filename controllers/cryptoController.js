...
const { getPaymentMethodViewData } = require('../utils/viewDataUtills');
...
router.get('/:cryptoId/edit', isAuth, async (req, res) => {
  ...
  const crypto = await cryptoService.getOne(req.params.cryptoId);
  const payment = getPaymentMethodViewData;
...
router.post('/:cryptoId/edit', isAuth, async (req, res) => {
  ...
  try {
    await cryptoService.edit(req.params.cryptoId, {
      name,
      image,
      price,
      description,
    ...
    return res
      .status(404)
      .render('crypto/edit', { crypto: crypto, error: getErrorMessage(error) });
  }
...
router.post('/create', isAuth, async (req, res) => {
  ...
  try {
    await cryptoService.create(userId, {
      name,
      image,
      price,
      description,
...
router.get('/search', async (req, res) => {
  let { name, payment } = req.query;

  const crypto = await cryptoService.search(name, payment);
  
  payment = getPaymentMethodViewData(payment);

  res.render('crypto/search', { crypto, payment, name });
});
